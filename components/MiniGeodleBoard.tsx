...
const MiniGeodleBoard: React.FC<MiniGeodleBoardProps> = ({ guesses, isFailed }) => {
  // Take top 3 guesses by similarity percentage, with safety check for move types
  const topGuesses = (guesses || [])
    .filter(g => g && g.type === 'geo-guess')
    .sort((a, b) => (b.percentage || 0) - (a.percentage || 0))
    .slice(0, 3);

  const hasWin = topGuesses.some(g => g.percentage >= 99.5);
    ...
    <div className="aspect-square w-full bg-white p-3 flex flex-col gap-2 rounded-xl border border-zinc-100 shadow-inner relative overflow-hidden">
      ...
      <div className="flex flex-col gap-1">
        {Array.from({ length: 3 }).map((_, i) => {
          const g = topGuesses[i];
          return (
            <div key={i} className="h-1 bg-zinc-50 rounded-full overflow-hidden border border-zinc-100/50">
              {g && (
                <div 
