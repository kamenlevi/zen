...
const WordleBoard: React.FC<WordleBoardProps> = ({ guesses, results, currentGuess, wordLength, maxGuesses }) => {
  ...
  return (
    <div className="flex flex-col gap-1 sm:gap-2 w-full items-center px-4 overflow-x-hidden no-scrollbar py-2 sm:py-6 flex-shrink">
      {rows.map((_, i) => {
            ...
            {Array.from({ length: wordLength }).map((_, j) => {
              ...
              return (
                <div
                  key={j}
                  className={`
                    w-[min(11vw,48px)] h-[min(11vw,48px)] sm:w-[76px] sm:h-[76px]
                    flex-shrink flex items-center justify-center 
                    text-base sm:text-4xl font-black uppercase rounded-none border-2 transition-all duration-300
                    ${bgColor} ${textColor}
