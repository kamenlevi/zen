...
export async function isValidWord(word: string): Promise<boolean> {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `Perform a deep linguistic analysis on the string "${w}". 
      Is it a real, valid 5-letter English word found in standard dictionaries (Oxford, Merriam-Webster)? 
      Be extremely strict. Reject random keyboard mashes, typos (e.g., 'babie' vs 'baby'), and non-English strings.
